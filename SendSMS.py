...
def sendsms(request):
    #try:
        ...
        d = request.json
        if len(d) == 3:
            sql = "select mobile  from new.appointment where business_id='"+str(d['business_id'])+"' and doctor_id='"+d['doctor_id']+"'"
        else:
            business_date= request.json['business_date']
            business_id = request.json['business_id']
            token_status = request.json['token_status']
            doctor_id = request.json['doctor_id']
            sql = "select mobile  from new.appointment where business_id='"+str(business_id)+"' and doctor_id='"+doctor_id+"'\
        ...
        return(json.dumps({"Message":"Sms Sent Successfully","Message Code":"SSS","Service Status":"Success"},indent =2))

   # except:
      #  return(json.dumps({"Message":"Sms Sent UnSuccessfully","Message Code":"SSUS","Service Status":"Failure"},indent=2))
